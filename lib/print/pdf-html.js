...
// Marketing/back-of-book promo placeholders. Override here when you change
// social handles or the App Store ID.
const APP_INSTAGRAM_HANDLE = '@mybooklab'
const APP_STORE_URL = 'https://apps.apple.com/app/id6761641708'
const APP_WEBSITE = 'mybooklab.app'

// Lulu minimum interior page count for the 8.5x8.5 SKUs we use.
...
// wide page. Caller passes spine width so we can compute the spread width.
// Trim is 8.5x8.5 + 0.125" bleed each edge → spread height = 8.75",
// spread width = 0.125 + 8.5 + spineWidthInches + 8.5 + 0.125.
export async function buildCoverHtml(book, { spineWidthInches }) {
  const cover = book.colors?.cover ?? '#8B5CF6'
  const accent = book.colors?.accent ?? '#06B6D4'
  const textColor = book.colors?.text ?? '#F1F5F9'

  const heightIn = 8.75
  const widthIn = 0.25 /* bleed both edges */ + 8.5 + spineWidthInches + 8.5
  ...
  const css = `
    ...
    .spread {
      ...
      display: grid;
      grid-template-columns: 8.625in ${spineWidthInches}in 8.625in;  /* 8.5 + 0.125 bleed each side */
      box-sizing: border-box;
